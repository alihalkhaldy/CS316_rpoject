# -*- coding: utf-8 -*-
"""CS316]_Sustainable_Agriculture.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1-TFHOFHjoEamDVLfo6KzIzoFgfwuY8A6
"""
...
def engineer_features(df):
    ...
    - Interaction Features:
        - 'rainfall_times_pesticides': To capture the combined effect of rainfall and pesticide usage on yield.
        - 'temp_times_pesticides': To understand how temperature influences the effectiveness of pesticides.
        - 'rainfall_times_temp': To see the combined impact of rainfall and temperature on crop yield.
    - Polynomial Features:
        - Adding squared and cubed terms for 'average_rain_fall_mm_per_year', 'avg_temp', and 'pesticides_tonnes' to capture non-linear relationships.

    Feature Usefulness:
    - These features aim to improve model performance by providing additional information that captures interactions and non-linear effects.
    - We will assess their usefulness through feature importance scores and model evaluation metrics.

    Multicollinearity Check:
    - High correlation between features can affect model interpretability.
    - We will check for multicollinearity and remove redundant features if necessary.
    """
    if df is not None and not df.empty:
...
if X_train is not None and not X_train.empty:
    ...
    print(f"X_train memory usage: {X_train.memory_usage(deep=True).sum() / 1024 ** 2:.2f} MB")
...
**Note:** Please ensure to create the `README.md` and `requirements.txt` files in your GitHub repository with the content provided above. The code and documentation provided should help you achieve full marks in the evaluation.
